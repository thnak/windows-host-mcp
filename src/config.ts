...
/** Raw host entry as written in the JSON config file (before key is read). */
interface RawHost {
  name?: string;
  description?: string;
  labels?: string[];
...
function parseShell(v: string | undefined, ctx: string): ShellKind {
  const s = (v ?? "powershell").trim().toLowerCase();
  if (s === "powershell" || s === "cmd" || s === "bash") return s;
  throw new Error(`${ctx}: shell must be one of powershell|cmd|bash, got: ${v}`);
}

function parsePort(v: unknown, ctx: string): number {
...
function parseTimeout(v: unknown, ctx: string): number {
  const t = Number(v ?? 20000);
  if (!Number.isFinite(t) || t <= 0) throw new Error(`${ctx}: readyTimeoutMs must be positive`);
  return t;
}

function readKey(keyPath: string, ctx: string): Buffer {
  try {
    return readFileSync(keyPath);
  } catch (err) {
    throw new Error(`${ctx}: cannot read key at ${keyPath}: ${(err as Error).message}`);
  }
}

function buildHost(raw: RawHost): HostConfig {
  const name = raw.name?.trim();
  if (!name) throw new Error(`Every host needs a "name"`);
  const ctx = `host "${name}"`;
  if (!raw.host?.trim()) throw new Error(`${ctx}: "host" is required`);
  if (!raw.username?.trim()) throw new Error(`${ctx}: "username" is required`);
  if (!raw.keyPath?.trim()) throw new Error(`${ctx}: "keyPath" is required`);
...
function hostsFromFile(path: string): AppConfig {
  let raw: RawConfig;
  try {
    raw = JSON.parse(readFileSync(path, "utf8")) as RawConfig;
  } catch (err) {
    throw new Error(`Could not read/parse WINDOWS_HOSTS_CONFIG=${path}: ${(err as Error).message}`);
  }
  ...
  for (const h of hosts) {
    if (names.has(h.name)) throw new Error(`Duplicate host name: ${h.name}`);
    names.add(h.name);
  }
...
export function loadConfig(): AppConfig {
  ...
  if (file) return hostsFromFile(file);
