 ...
 */
function wrapPowershell(command: string, opts: WrapOptions): string {
  const lines: string[] = [];
 ...
 */
function wrapCmd(command: string, opts: WrapOptions): string {
  const parts: string[] = [];
  if (opts.cwd) parts.push(`cd /d "${opts.cwd}"`);
  for (const [k, v] of Object.entries(opts.env ?? {})) {
    parts.push(`set "${k}=${v}"`);
  }
  parts.push(command);
...
function wrapBash(command: string, opts: WrapOptions): string {
  const parts: string[] = [];
  if (opts.cwd) parts.push(`cd ${bashSingleQuote(opts.cwd)}`);
  for (const [k, v] of Object.entries(opts.env ?? {})) {
