...
function bashq(value: string): string {
  return `'${value.replace(/'/g, "'\\''")}'`;
}
...
function toolProbeScript(tools: string[], emit: string): string {
...
export async function checkTools(conn: SshConnection, tools: string[]): Promise<ToolInfo[]> {
  ...
  try {
    return asArray<ToolInfo>(JSON.parse(res.stdout.trim() || "[]"));
  } catch {
...
export async function getHostInfo(conn: SshConnection, tools: string[]): Promise<HostInfo> {
  const emit = `
$out = [ordered]@{
...
$out | ConvertTo-Json -Depth 5`;
  ...
  return {
    ...
    tools: asArray<ToolInfo>(parsed.tools),
  };
}
