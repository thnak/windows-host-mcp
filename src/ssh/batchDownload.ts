...
async function buildRemoteZip(
  conn: SshConnection,
  remoteDir: string,
  selection: BatchDownloadSelection,
): Promise<RemoteSelectResult> {
...
if ($items.Count -eq 0) {
  ...
  if (!raw) throw new Error("No output from remote file-selection script.");
  let parsed: RemoteSelectResult;
  try {
...
async function cleanupRemoteZip(conn: SshConnection, remoteZipPath: string): Promise<void> {
...
function summarize(files: ExtractFileResult[]): BatchDownloadResult {
  return {
    files,
...
export async function batchDownload(
  ...
  if (!remote.zipPath) return summarize([]);
