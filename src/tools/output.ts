...
function nextFile(kind: string, ext: string): string {
  fileCounter += 1;
 ...
 */
export function tailAndSave(full: string, kind: string, ext = "log"): TailResult {
 ...
 */
export function capAndSave<T>(entries: T[], kind: string): CapResult<T> {
