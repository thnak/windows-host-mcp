...
export function registerFileTools(server: McpServer, ctx: AppContext): void {
  server.registerTool(
    ...
    async ({ localPath, remotePath, host }) => {
      try {
        ...
        return result(
          { host: target.name, uploaded: true, remotePath: remote },
          `Uploaded to ${remote} on ${target.name}.`,
        );
      } catch (err) {
  ...
  server.registerTool(
    ...
    async ({ remotePath, localPath, host }) => {
      try {
        ...
        return result(
          { host: target.name, downloaded: true, localPath: local },
          `Downloaded to ${local} from ${target.name}.`,
        );
      } catch (err) {
        return errorResult(err);
      }
    ...
    "list_remote_dir",
    {
      ...
      inputSchema: {
        ...
        truncate: z
          ...
          .optional()
          .describe(
            "Cap the number of entries returned inline and spill the full listing to a temp file instead of flooding the reply. Default true; pass false to return every entry inline.",
          ),
      },
    },
  ...
  server.registerTool(
    "batch_upload",
    {
      ...
      description:
        "Upload many local files to a Windows host in one shot. Select files by an explicit list, a regex " +
      ...
      inputSchema: {
        ...
        files: z
          ...
          .optional()
          .describe(
            "Explicit file paths relative to localDir to upload as-is. When set, `pattern` and `recursive` are ignored.",
          ),
        pattern: z
          .string()
          .optional()
          .describe(
            "Regex tested against each candidate file's path relative to localDir (forward-slash separated). " +
    ...
    async ({ localDir, files, pattern, recursive, remoteDir, writePolicy, host }) => {
      try {
        ...
        if (selected.length === 0) {
          return result(
            { host: target.name, remoteDir, writePolicy: policy, filesMatched: 0 },
            "No local files matched the selection; nothing to upload.",
          );
        }
  ...
  server.registerTool(
    "batch_download",
    {
      ...
      description:
        "Download many files from a Windows host in one shot — the mirror of batch_upload. Select files by an " +
      ...
      inputSchema: {
        ...
        files: z
          ...
          .optional()
          .describe(
            "Explicit file paths relative to remoteDir to download as-is. When set, `pattern` and `recursive` are ignored.",
          ),
        pattern: z
          .string()
          .optional()
          .describe(
            "Regex tested against each candidate file's path relative to remoteDir (forward-slash separated). " +
              "Ignored when `files` is set.",
          ),
        recursive: z
          .boolean()
    ...
    async ({ remoteDir, files, pattern, recursive, localDir, writePolicy, host }) => {
      try {
        ...
        if (summary.files.length === 0) {
          return result(
            { host: target.name, remoteDir, localDir, writePolicy: policy, filesMatched: 0 },
            "No remote files matched the selection; nothing to download.",
          );
        }
