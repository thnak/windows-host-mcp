...
export function registerProbeTools(server: McpServer, ctx: AppContext): void {
  server.registerTool(
    "get_host_info",
    {
    ...
    async ({ host, tools }) => {
      try {
        ...
        let sessionCwd: string | null = session.cwd ?? null;
        let sessionCwdValid: boolean | null = null;
        if (session.cwd) {
        ...
        const text =
          `[${target.name}] ${info.os ?? "Windows"} (${info.osVersion ?? "?"}) — ${info.user ?? "?"}@${info.hostname ?? "?"}\n` +
  ...
  server.registerTool(
    ...
    async ({ tools, host }) => {
      try {
        ...
        const text = results
          .map((t) =>
            t.found ? `${t.name}: ${t.version ?? "(found)"} — ${t.path}` : `${t.name}: NOT FOUND`,
          )
          .join("\n");
