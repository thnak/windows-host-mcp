      ...
      try {
        ...
        if (cwd !== undefined) {
          ...
          if (!ok) {
            return errorResult(
              new Error(
                `cwd "${cwd}" does not exist (or is not a directory) on host ${target.name}. ` +
