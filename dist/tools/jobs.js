...
export function registerJobTools(server, ctx) {
    ...
    server.registerTool("get_job_output", {
        ...
        description: "Fetch output from a running or finished job (any host — the jobId identifies it). Pass sinceOffset (the nextOffset from the previous call) to get only new output. Includes status and exit code once the job finishes.",
        inputSchema: {
            ...
            truncate: z
                .boolean()
                .optional()
                .describe("Tail the new output and spill the full slice to a temp file (.whmcp-tmp/) instead of flooding the reply. Default true; pass false to force the entire slice inline. nextOffset still advances past everything, so the next poll only returns newer output."),
        },
    }, async ({ jobId, sinceOffset, truncate }) => {
        try {
            const out = ctx.manager.getJobOutput(jobId, sinceOffset ?? 0);
            const tail = truncate === false
    ...
    server.registerTool("send_job_input", {
        title: "Send stdin to a job",
        description: "Write input to a running job's stdin — e.g. answer a prompt or feed a REPL. A newline is appended by default.",
        inputSchema: {
    ...
    server.registerTool("cancel_job", {
        ...
        description: "Stop a running job. Sends a signal and closes the channel. Note: signal delivery is unreliable on Windows OpenSSH; for stubborn processes run a taskkill via run_command.",
        inputSchema: {
            jobId: z.string().describe("The job id."),
    ...
    server.registerTool("list_jobs", {
        title: "List jobs",
        description: "List jobs with their host, status, and exit codes. Defaults to all hosts; pass host to filter.",
        inputSchema: {
