...
export function registerRunCommandTools(server, ctx) {
    server.registerTool("test_connection", {
        title: "Test a Windows SSH connection",
        description: "Verify the SSH connection to a host and return its hostname and current user. Defaults to the active host. Use this first to confirm setup.",
    ...
    server.registerTool("run_command", {
        ...
        description: "Run a one-shot command on a Windows host and wait for it to finish. Returns stdout, stderr, and exit code. Defaults to the active host and its session shell/cwd/env; use start_job instead for long-running or interactive commands.",
        inputSchema: {
            ...
            truncate: z
                .boolean()
                .optional()
                .describe("Tail long output and spill the full stdout/stderr to a temp file (.whmcp-tmp/) instead of flooding the reply. Default true; pass false to force the entire output inline."),
        },
