...
function loadHostsFile(path) {
    ...
    try {
        ...
        throw new Error(`Could not parse existing config at ${path}: ${err.message}`);
    }
    const obj = parsed;
    return { defaultHost: obj.defaultHost, hosts: Array.isArray(obj.hosts) ? obj.hosts : [] };
}
function saveHostsFile(path, data) {
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, `${JSON.stringify(data, null, 2)}\n`, "utf8");
}
async function promptHost(p, existing) {
    ...
    const keyPath = await p.ask("Private key path (on this machine)", existing?.keyPath ?? join(homedir(), ".ssh", "windows_host_ed25519"));
    if (!host || !username || !keyPath)
        throw new Error("host, username, and keyPath are required.");
    ...
    const keyPassphrase = await p.ask("Private key passphrase (optional, stored in plaintext in the config file)", existing?.keyPassphrase);
    return {
        name,
        host,
        username,
...
async function registerWithClaude(p, configPath) {
    ...
    const add = spawnSync("claude", ["mcp", "add", "windows-host-mcp", "-s", scope, "-e", `WINDOWS_HOSTS_CONFIG=${configPath}`, "--", "windows-host-mcp"], { stdio: "inherit" });
    if (add.status === 0) {
...
export async function runConfig(args) {
    ...
    const configPath = pathFlagIdx >= 0 && args[pathFlagIdx + 1]
        ? args[pathFlagIdx + 1]
    ...
    try {
        ...
        while (editing) {
            ...
            if (action.startsWith("a")) {
                ...
                if (data.defaultHost === target)
                    data.defaultHost = undefined;
                saveHostsFile(configPath, data);
